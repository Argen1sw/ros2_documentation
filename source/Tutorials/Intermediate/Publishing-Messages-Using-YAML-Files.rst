...
We assume that an entity is publishing `a geometry_msgs/msg/Twist message <{interface_link(geometry_msgs/msg/Twist)}>`_ message through a topic named ``cmd_vel`` and we want to capture the message, edit it and publish it to a topic.
